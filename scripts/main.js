...
function drawBars(unsorted_array) {
  bars_container.innerHTML = "";//These two lines clear any existing content within the 
  bars_value_container.innerHTML = "";//bars_container and bars_value_container elements

  for (let i = 0; i < unsorted_array.length; i++) {
    // Render Bars
    bars_array[i] = document.createElement("div");//A new div element is created for each bar in the bar_array
    bars_container.appendChild(bars_array[i]);

    bars_array[i].style = `margin: 0 ${margin_size}%; height: ${ unsorted_array[i] * 10}px; width: ${100 / numOfBars - 2 * margin_size}%;
