...
function selectionSort() {
  ...
  for (var i = 0; i < numOfBars - 1; i++) {
    update_bar(bars_array[i], unsorted_array[i], "#cc9900"); //Color update
    ...
    for (var j = i + 1; j < numOfBars; j++) {
      ...
      if (unsorted_array[j] < unsorted_array[index_min]) {
        ...
        update_bar(bars_array[index_min], unsorted_array[index_min], "#cc9900"); //Color update
      } else {
    ...
    if (index_min != i) {
      ...
      update_bar(bars_array[index_min], unsorted_array[index_min], "#cc9900"); // Height update
      update_bar(bars_array[i], unsorted_array[i], "#cc9900"); // Height update
